...
class SettingsWindow:
    ...
    def build_ui(self):
        ...
        self.timeout_var = tk.StringVar(value=str(config.get("timeout", 1.5)))
        ...
        self.active_provider_var = tk.StringVar(value=config.get("ai_provider", "openai"))
        
        self.provider_cb = ttk.Combobox(
            provider_frame, 
            textvariable=self.active_provider_var, 
            values=[self.providers[k]["name"] for k in self.provider_keys],
            state="readonly"
        )
        ...
        ttk.Entry(self.prov_config_frame, textvariable=self.endpoint_var, width=45).grid(row=0, column=1, sticky=tk.E, pady=2)

        ttk.Label(self.prov_config_frame, text="API Key:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.apikey_var = tk.StringVar()
        ttk.Entry(self.prov_config_frame, textvariable=self.apikey_var, width=45, show="*").grid(row=1, column=1, sticky=tk.E, pady=2)
    ...
    def on_provider_change(self, event=None):
        # Save current to dict first if we are switching
        if hasattr(self, '_current_provider_key'):
            self.providers[self._current_provider_key]["endpoint"] = self.endpoint_var.get()
            self.providers[self._current_provider_key]["api_key"] = self.apikey_var.get()

        selected_name = self.active_provider_var.get()
        # Find key by name
        for k, v in self.providers.items():
            if v["name"] == selected_name:
                self._current_provider_key = k
                self.endpoint_var.set(v.get("endpoint", ""))
                self.apikey_var.set(v.get("api_key", ""))
                break
...
def show_settings(is_welcome=False, on_close_callback=None):
    app = SettingsWindow(is_welcome, on_close_callback)
    app.root.mainloop()
